...
ok('blank when prob below gate', rainToken(w(5),[10])===null);
ok('blank when dry', rainToken(w(0),[90])===null);
ok('maybe wet 0.5-2', rainToken(w(1),[80])==='maybe wet', rainToken(w(1),[80]));
ok('wet 2-6', rainToken(w(3),[80])==='wet', rainToken(w(3),[80]));
ok('very wet >6', rainToken(w(8),[80])==='very wet', rainToken(w(8),[80]));
ok('boundary 0.5 -> maybe', rainToken(w(0.5),[80])==='maybe wet');
ok('just under 0.5 -> blank', rainToken(w(0.49),[80])===null);
// time weighting: heavy rain only on a tiny segment shouldn't dominate
const mixed=[{rate:12,t:60},{rate:0,t:1200}]; // 1 min heavy, 20 min dry
ok('brief heavy averaged down', rainToken(mixed,[90])===null, rainToken(mixed,[90]));

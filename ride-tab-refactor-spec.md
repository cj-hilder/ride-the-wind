...
- **Inaudible audio + MediaSession — REMOVED / not used.** The W3C Geolocation
  spec gates `watchPosition` on **document visibility**, and screen lock (or any
  backgrounding) makes the document `hidden`, so `watchPosition` stops delivering
  regardless of any keep-alive. Audio/MediaSession keeps the JS process alive and
  unthrottled but does **not** exempt geolocation from the visibility gate (audio
  is on the throttling-exemption list; geolocation is not). This is cross-engine,
  spec-level, intended behaviour — there is **no pure-PWA way** to record GPS with
  the screen off (the only path is a native wrapper / TWA + foreground service).
  So the route recorder uses **Wake Lock only** (keep the screen on → document
  stays visible → GPS flows) plus on-screen guidance: "Keep this app open and
  visible while recording. The ability to record GPS data in the background is not
  currently available." Background recording is noted as a possible future
  enhancement (would require migrating to a native, e.g. Flutter-based, app).
